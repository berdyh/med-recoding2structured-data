...
from .llm_provider import LLMProviderError, LLMProviderManager
...
def parse_arguments():
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="GP Consultation Data Extraction System")

    parser.add_argument(
        "--input", type=str, required=True, help="Path to input consultation Markdown file"
    )
    ...
    parser.add_argument("--patient-id", type=str, help="Patient UUID (if not using test data)")

    parser.add_argument("--doctor-id", type=str, help="Doctor UUID (if not using test data)")
...
def _map_all_entities(mapper, extracted_entities, transcript):
    ...
    try:
        ...
        mapped_data["consultation_sessions"] = mapper.generate_consultation_session(
            transcript, {"session_start_datetime": current_timestamp}
        )

        # Map consultation
        mapped_data["consultations"] = mapper.generate_consultation(
            {"consultation_datetime": current_timestamp, "consultation_type": "GP Consultation"}
        )

        # Map all entity types
        mapped_data["symptoms"] = mapper.map_symptoms(extracted_entities.get("symptoms", []))
        mapped_data["medications"] = mapper.map_medications(
            extracted_entities.get("medications", [])
        )
        ...
        mapped_data["vital_signs"] = mapper.map_vital_signs(
            extracted_entities.get("vital_signs", [])
        )
    ...
    try:
        ...
        try:
            ...
            manifest_path = exporter.create_manifest(
                str(consultation_session_id), total_entities, processing_time
            )
