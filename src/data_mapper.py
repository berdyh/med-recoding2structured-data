...
class DataMapper:
    ...
    def __init__(self, consultation_session_id: uuid.UUID, patient_id: uuid.UUID, doctor_id: uuid.UUID):
    ...
    def _group_entities_by_attribute(self, entities: List[Dict], group_attr: str) -> Dict[str, List[Dict]]:
    ...
    def map_symptoms(self, symptoms: List[Dict]) -> pd.DataFrame:
        ...
        if not symptoms:
            ...
            return pd.DataFrame(columns=[
                ...
                'symptom_name', 'symptom_code', 'severity', 'duration_days',
                'onset_date', 'temporal_pattern', 'confidence_score',
                'clinical_modifiers', 'extracted_at'
            ])
    ...
    def map_medications(self, medications: List[Dict]) -> pd.DataFrame:
        ...
        if not medications:
            return pd.DataFrame(columns=[
                'prescription_id', 'consultation_session_id', 'patient_id', 'doctor_id',
                'medication_name', 'dosage_value', 'dosage_unit', 'frequency',
                'duration_value', 'duration_unit', 'route_of_administration',
                'indication', 'prescribed_at'
            ])
    ...
    def map_diagnoses(self, diagnoses: List[Dict]) -> tuple:
        ...
        """
        if not diagnoses:
            diagnoses_df = pd.DataFrame(columns=[
                'diagnosis_id', 'consultation_session_id', 'patient_id', 'doctor_id',
                'diagnosis_name', 'diagnosis_code', 'diagnostic_certainty',
                'clinical_notes', 'diagnosed_at'
            ])
            assessment_df = pd.DataFrame(columns=[
                'assessment_id', 'consultation_session_id', 'patient_id', 'doctor_id',
                'assessment_diagnosis', 'diagnostic_certainty', 'diagnostic_reasoning',
                'key_clinical_features_supporting', 'assessed_at'
            ])
    ...
    def map_vital_signs(self, vitals: List[Dict]) -> pd.DataFrame:
        ...
        if not vitals:
            return pd.DataFrame(columns=[
                'vital_sign_id', 'consultation_session_id', 'patient_id',
                'temperature', 'blood_pressure', 'heart_rate',
                'oxygen_saturation', 'respiratory_rate', 'measured_at'
            ])
    ...
    def _parse_temperature(self, temp_text: str) -> Optional[float]:
    ...
    def map_physical_exam(self, exams: List[Dict]) -> pd.DataFrame:
        ...
        if not exams:
            return pd.DataFrame(columns=[
                'exam_finding_id', 'consultation_session_id', 'patient_id',
                'examination_type', 'anatomical_site', 'findings',
                'severity', 'examined_at'
            ])
    ...
    def map_red_flags(self, flags: List[Dict]) -> pd.DataFrame:
        ...
        if not flags:
            return pd.DataFrame(columns=[
                'red_flag_id', 'consultation_session_id', 'patient_id',
                'warning_description', 'warning_type', 'severity', 'identified_at'
            ])
    ...
    def map_follow_up(self, plans: List[Dict]) -> pd.DataFrame:
        ...
        if not plans:
            return pd.DataFrame(columns=[
                'follow_up_id', 'consultation_session_id', 'patient_id',
                'follow_up_type', 'timing', 'condition', 'action',
                'priority', 'planned_at'
            ])
